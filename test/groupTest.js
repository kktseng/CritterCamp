...
function getRedisInfo(group, users, callback) {
  var commands = [
    ...
    async.apply(redis.smembers.bind(redis), 'queue_test_version')
  ];
  ...
  describe('Single User', function() {
    ...
    it('can create a new group if no groups are available', function(done) {
      redis.hset('user_test_user1', 'version', 'test_version', function(err) {
        if(err) { return done(err); }
        group.findGroup(['test_user1'], function(err, group_id) {
          if(err) { return done(err); }
          group_id.should.eql('1');
          getRedisInfo(1, 'test_user1', function(err, results) {
            if(err) { return done(err); }
    ...
    it('can put a user in a group waiting on users', function(done) {
      async.parallel([
        ...
        async.apply(redis.sadd.bind(redis), 'queue_test_version', '2'),
        async.apply(redis.hset.bind(redis), 'user_test_user2', 'version', 'test_version')
      ], function(err) {
        if(err) { return done(err); }
        group.findGroup(['test_user2'], function(err, group_id) {
          if(err) { return done(err); }
    ...
    it('can complete a group with a single user', function(done) {
      async.parallel([
        ...
        async.apply(redis.sadd.bind(redis), 'queue_test_version', '2'),
        async.apply(redis.hset.bind(redis), 'user_test_user4', 'version', 'test_version')
      ], function(err) {
        if(err) { return done(err); }
        group.findGroup(['test_user4'], function(err, group_id) {
          if(err) { return done(err); }
          group_id.should.eql('2');
  ...
  describe('Party', function() {
    ...
    it('can add a party to an existing group', function(done) {
      async.parallel([
        ...
        if(err) { return done(err); }
        group.findGroup(['test_user2', 'test_user3'], function(err, group_id) {
          if(err) { return done(err); }
          group_id.should.eql('2');
    ...
    it('can complete a group with a party', function(done) {
      async.parallel([
        ...
        if(err) { return done(err); }
        group.findGroup(['test_user2', 'test_user3', 'test_user4'], function(err, group_id) {
          if(err) { return done(err); }
          group_id.should.eql('2');
    ...
    it('can create a finished group if party size == maxPartySize', function(done) {
      redis.hset('user_test_user1', 'version', 'test_version', function(err) {
        if(err) { return done(err); }
        group.findGroup(['test_user1', 'test_user2', 'test_user3', 'test_user4'], function(err, group_id) {
          if(err) { return done(err); }
          group_id.should.eql('1');
