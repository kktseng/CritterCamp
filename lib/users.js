...
var checkLeaderboard = function(new_level, callback) {
  helpers.aquireLock('leaderboard', 3, function(err, obtained) {
    if(err) { return callback(err); }
    if(!obtained) { return callback(null, false); }
    var num_leaders = 0;
    ...
    var release_and_finish = function(callb) {
      update_leaderboard(function(err) {
        if(err) { return callb(err); }
        helpers.releaseLock('leaderboard', function(err) {
          return callb(err, true);
        });
      });
    };
...
var updateRanks = function(old_level, new_level, callback) {
  ...
  async.forEach(change_levels, helpers.m.Rank.incrRank, function(err) {
    if(err) { return callback(err); }
    // check if the leaderboard needs to be updated every 100ms in case of lock
    var leaderboard_checked = false;
    async.until(function() { return leaderboard_checked; }, function(cb) {
      setTimeout(function() {
        checkLeaderboard(new_level, function(err, completed) {
          if(err) { return cb(err); }
          leaderboard_checked = completed;
          cb(null);
        });
      }, 100);
    }, callback);
