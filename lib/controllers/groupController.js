...
module.exports = function(username, message, callback) {
  if(message.type === 'join') {
    group.findGroup([username], function(err, group_id, size) {
      if(!err && group_id) {
        ...
        if(size == globals.MAX_PARTY_SIZE) {
          ...
          group.messageGroup(group_id, JSON.stringify({ action: 'group', type: 'count', size: size }), callback);
          helpers.redis.multi()
    ...
    group.removeFromGroup(username, function(err, group_id) {
      ...
      helpers.redis.lrange('group_' + group_id, 0, -1, function(err, users) {
        if(err) { return callback(err); }
        if(users) {
          group.messageGroup(group_id, JSON.stringify({ action: 'group', type: 'count', size: users.length }), callback);
        }
      });
    });
