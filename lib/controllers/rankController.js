...
/*
    helpers.m.User.getUser(username, function(err, user) {
      users.setExp(username, user.exp + message.exp_gained, function(err) {
        if(err) { return callback(err); }
        helpers.m.User.getUser(username, function(err, updated_user) {
          if(err) { return callback(err); }
          result.level = updated_user.level;
          //SET % TO NEXT LEVEL HERE
          helpers.m.User.setGold(updated_user.gold + gold_gained, function(err) {
            if(err) { return callback(err); }
            users.messageUser(username, JSON.stringify(reply), callback);
          });
        });
