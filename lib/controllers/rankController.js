...
module.exports = function(username, message, callback) {
  if (message.type == 'leader') {
    ...
    async.waterfall([
      ...
      function(user, cb) {
        users.setExp(username, user.exp + message.exp_gained, cb);
      },
      function(updated_user, cb) {
        reply.level = updated_user.level;
        reply.percentage = updated_user.getPercentNextLevel();
        helpers.m.User.setGold(updated_user.gold + message.gold_gained, cb);
      }
    ], function(err) {
      if(err) { return callback(err); }
      users.messageUser(username, JSON.stringify(reply), callback);
