  ...
  .exec(function(err, reply) {
    ...
    if(reply[0] === reply[1]) {
      ...
      if(reply[1] === 1) {
        helpers.redis.multi()
          .incr('group_' + group_id + '_sync_id')
          .expire('group_' + group_id + '_sync_id', globals.GROUP_TIMEOUT) // 5 min timeout
        .exec(function(err, reply) {
          if(err) { return callback(err); }
          var sync_id = reply[0];
            ...
            helpers.redis.get('group_' + group_id + '_sync_id', function(err, val) {
              ...
              if(val == sync_id) {
                getNonSyncUsers(function(err, remove_users) {
                  if(err) { return callback(err); }
                  async.each(remove_users, disconnectUser, function(err) {
                    if(err) { return callback(err); }
                    sendReply(callback);
                  });
                });
...
exports.setupGame = function(usernames, callback) {
  var user_info = [];
  async.each(usernames, function(username, callb) {
    async.waterfall([
      function getInfo(cb) {
        helpers.m.User.getUserInfo(username, function(err, info) {
          if(err) { return cb(err); }
          user_info.push(info);
          cb(null);
        });
...
exports.findGroup = function(usernames, callback) {
  ...
  redis.hget('user_' + usernames[0], 'version', function(err, version) {
    ...
    if(party_size > globals.MAX_PARTY_SIZE) {
      logger.warn('Party ' + usernames + ' was too large for game.');
      ...
      redis.llen('group_' + group_id, function(err, length) {
        ...
        if(length + party_size <= globals.MAX_PARTY_SIZE) {
          ...
          usernames.forEach(function(username) {
            ...
            multi.expire('group_' + group_id, globals.GROUP_TIMEOUT);
          });
          ...
          multi.exec(function(err, reply) {
            ...
            if(reply) { // the group was successfully added onto
              return cb({ success: group_id, size: length + party_size });
            }
    ...
    async.waterfall([
      ...
      function checkOpenGroups(arr, cb) {
        async.eachSeries(arr, checkAvailability, cb);
      },
      // could not add to any group, so create a new one
      async.apply(helpers.createId),
      function insertInGroup(group_id, cb) {
        async.series([
          async.apply(async.each, usernames, function(user, callb) {
            redis.multi()
              .rpush('group_' + group_id, user)
              .expire('group_' + group_id, globals.GROUP_TIMEOUT)
            .exec(callb);
          }),
          ...
          if(err) { return cb(err); }
          cb({ success: group_id, size: results[1] });
        });
      }
    ], function(err) {
      if(err && err.success) {
        ...
        async.each(usernames, function(username, cb) {
          ...
          return callback(err, group_id, size);
        });
      } else {
        return callback(err);
      }
...
var removeFromGroup = exports.removeFromGroup = function(username, callback) {
  ...
  async.waterfall([
    ...
    function(status, cb) {
      redis.lrange('group_' + group, 0, -1, cb);
    }
  ], function(err, users) {
    if(err) { return callback(err); }
      if(users) {
        messageGroup(group, JSON.stringify({ action: 'group', type: 'count', size: users.length }), function(err) {
          callback(err, group);
        });
      } else {
        callback(err, group);
...
exports.deleteGroup = function(group_id, callback) {
  redis.lrange('group_' + group_id, 0, -1, function(err, user_list) {
    if(err) { return callback(err); }
    async.each(user_list, function(username, cb) {
