...
exports.findGroup = function(user_id, game, callback) {
  ...
  if(party_size > GROUP_SIZE) {
    ...
    helpers.createId(function(err, group_id) {
      if(err) { return callback(err); }
      for(var i = 0; i < user_id.length; i++) {
        redis.rpush('group_' + group_id, user_id[i]);
      }
      user.messageUser(user_id, 'Congrats bro, youre in group ' + group_id);
    });
  } else {
    ...
    async.waterfall([
      ...
      if(err && err.success) {
        ...
        if(full) {
          redis.lrange('group_' + err.success, 0, -1, function(error, users) {
            if(error) { callback(error); }
            user.messageAllUsers(users, 'Congrats bro, youre in group ' + err.success + ' with ' + users);
...
* cb(err, group from which user_id was deleted from)
**/
exports.removeFromGroup = function(user_id, callback) {
  async.waterfall([
    function(cb) {
      redis.hget('user_' + user_id, 'group', cb);
    },
    function(group_id, cb) {
      redis.lrem('group_' + group_id, 0, user_id);
      cb(null, group_id);
    }
  ], function(err, group_id) {
    if(err) { callback(err); }
    redis.hdel('user_' + user_id, 'group');
