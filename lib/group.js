...
* callback(err)
*/
function sync(group_id, data, timeout, final, callback) {
  helpers.redis.multi()
    .llen('group_' + group_id)
    .rpush('group_' + group_id + '_sync', JSON.stringify(data))
    .expire('group_' + group_id + '_sync', timeout + 5)
...
* callback(err)
*/
exports.syncGroup = function(group_id, data, timeout, callback) {
  sync(group_id, data, timeout, function(reply, cb) {
    var aggregated = [];
...
exports.handleScore = function(group_id, score_data, callback) {
  sync(group_id, score_data, 10, function(scores, cb) {
    score_map = {};
...
exports.setupGame = function(usernames, callback) {
  ...
  function updateMetrics(username, cb) {
    helpers.m.User.getUser(username, function(err, user) {
      // ignore err; don't want metrics err to affect joining game
      if(user) {
        user.totalGames++;
        user.save(cb);
      }
  ...
  async.forEach(usernames, getUserInfo, function(err) {
    if(err) { return callback(err); }
    helpers.redis.multi()
    ...
    .exec(function(err) {
      if(err) { return callback(err); }
      async.forEach(usernames, updateMetrics, function(err) {
        if(err) { return callback(err); }
