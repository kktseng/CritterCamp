...
exports.selectGames = function(version, callback) {
  helpers.m.Game.find({ minVersion: { $lte: version } }, function(err, results) {
    if(err) { return callback(err); }
	...
	console.log('selected!!!' + selected);
      ...
      var temp = results[current_length-1];
      results[current_length-1] = results[selected];
      results[selected] = temp;

      current_length--;
    }
	console.log('resultsfinal!!!'+results);
    return callback(null, results.slice(current_length, results.length));
  });
};
