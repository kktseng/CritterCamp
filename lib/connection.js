  ...
  helpers.createId(function(err, conn_id) {
    ...
    connection.on('close', function(data) {
      ...
      async.series([
        ...
        async.apply(helpers.redis.decr.bind(helpers.redis), 'connections_count')
    ...
    connection.on('data', function(message) {
      ...
      messages.forEach(function(message) {
        ...
        if(!auth) {
          ...
          if(controllers[message.action] !== undefined) {
            controllers[message.action](username, message, logger.error);
          }
        }
      });
    });
    ...
    messageConn(conn_id, JSON.stringify(conn_reply), function(err) {
      if(err) { return logger.error(err); }
      helpers.redis.incr('connections_count', function(err) {
        if(err) { return logger.error(err); }
      });
    });
  });
