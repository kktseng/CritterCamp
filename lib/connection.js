              ...
              helpers.redis.hget('user_' + username, 'conn', function(err, conn) {
                ...
                if(conn) {
                  // wipe any existing user data
                    // remove user's group_id from group key
                    // if this causes there to be no more users in group, remove that group_id from queue
                    // remove user's party_id from party key
