...
exports.messageLocalConn = function(conn_id, message) {
  if(connection_list[conn_id]) {
    try {
      ...
      logger.warn('Unable to write: ' + message);
...
exports.request = function(connection) {
  ...
  // helps put split packets together
  var curr_packet = '';
  ...
  helpers.createId(function(err, conn_id) {
    ...
    connection.on('error', function(exc) {
      logger.warn(exc);
      helpers.redis.decr('connection_count', logger.err);
    });
    ...
    var handleMessage = function(message) {
      try {
        if(message === '{}') {
          ...
          if(curr_packet !== '' && curr_packet !== message) {
            logger.warn('Problem handing packet: ' + curr_packet);
          }
          curr_packet = '';
        }
      } catch(err) {
      ...
      if(!auth) {
        if(message.auth) {
          helpers.redis.hget('auth', message.auth, function(err, name) {
            ...
            logger.info(moment().format('hh:MM:SS a: ') + 'User ' + username + ' logged in!!!');
          });
        }
      } else {
