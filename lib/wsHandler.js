...
var deleteConn = exports.deleteConn = function(conn_id) {
  var message = { 'delete': conn_id };
  helpers.publisher.publish('connection', JSON.stringify(message));
};
...
exports.messageConn = function(conn_id, message) {
  ...
  if(conn_id instanceof Array) {
    ...
    for(var i = 0; i < conn_id.length; i++) {
      message.message.push({ conn_id: conn_id[i], msg: message });
    }
  } else {
    message = { message: [{ conn_id: conn_id, msg: message }] };
  }
  helpers.publisher.publish('connection', JSON.stringify(message));
};
