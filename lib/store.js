...
exports.increaseGameLevel = function(username, game_name, index, callback) {
  ...
  helpers.m.User.findOne({ username: username, 'gameUpgrades.game': game_name, 'gameUpgrades.index': index, 'gameUpgrades.level': { $gte: globals.STORE.game_upgrades.length - 1 }}, function(err, doc) {
    if(err) { return callback(err); }
    if(doc) {
