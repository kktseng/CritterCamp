...
var increaseGameLevel = exports.increaseGameLevel = function(username, game_name, index, callback) {
  // check if level already maxed out
  helpers.m.User.findOne({ username: username, gameUpgrades: { $elemMatch: { game: game_name, index: index, level: { $gte: globals.STORE.game_upgrades.length }}}}, function(err, doc) {
    if(err) { return callback(err); }
    if(doc) {
